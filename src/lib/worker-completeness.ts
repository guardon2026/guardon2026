...
/**
 * 경비 인력이 SOS에 신청·수락하기 전에 반드시 채워야 하는 항목(성명·연락처·주소·
 * 은행명·계좌번호·예금주)이 모두 있는지 판정한다. bankVerifiedAt은 관련 필드들과
 * 함께 원자적으로 세팅되므로 타임스탬프만 확인하면 하위 필드도 채워졌다고 볼 수 있다.
 * workFields(업무 분야)는 매칭 조건이라 여기 포함하지 않고 호출부에서 별도로 체크한다.
 */
